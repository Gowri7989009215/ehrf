  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (uploadType === 'file' && selectedFile) {
        ...
        toast.error('Please provide either a file or text data');
        return;
      }

      const response = await axios.post('/patient/upload', formData, {
        headers: {
      ...
      const message = error.response?.data?.message || 'Failed to upload record';
      toast.error(message);
    } finally {
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600">
          Add a new medical record to your secure health profile
        </p>
      </div>
          ...
          <div className="card-body space-y-4">
            ...
            <div>
              ...
              <input
                ...
                type="text"
                className="form-input"
                placeholder="e.g., blood, cholesterol, annual (comma-separated)"
                {...register('tags')}
              />
              <p className="text-sm text-gray-500 mt-1">
                Add tags to help organize and search your records
              </p>
            </div>
          </div>
        </div>
            ...
            {uploadType === 'file' ? (
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Enter or paste your medical record content here..."
                  {...register('textData', {
                    required: uploadType === 'text' ? 'Content is required' : false
                  })}
                />
                {errors.textData && (
                  <p className="form-error">{errors.textData.message}</p>
                )}
              </div>
            )}
          </div>
